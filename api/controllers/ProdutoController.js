...
class ProdutoController {
    ...
    async index(req, res, next) {
        ...
        const limit = Number(req.query.offset) || 30;
        try {
    ...
    async search(req, res, next) {
        ...
        const search = new RegExp(req.params.search, "i"); // "i" indica que é case INsensitive
        // $or faz uma consulta em todos os itens listados. Uma busca que procura em todos os campos listados.
        // $regex faz uma busca onde procura ALGUMA semelhança na string buscada e no BD
        try {
            const produtos = await Produto.paginate(
                {
                    loja: req.query.loja,
